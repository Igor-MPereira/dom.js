...
const COLON_MATCH_AND_CAPTURE = /(?<!\\)(:)/g;
...
export function FormatCSSString(string, config) {
  ...
  let IL = 0;
  const tabSize = (config && config.tabSize) || TAB_SIZE;
  const spacing = (config && config.spacing) || SPACING;

  let result = string;
  result = HandleString(string, tabSize, spacing);
...
function HandleString(string, tabSize, spacing) {
  ...
  let IL = 0;
  return string.split(CURLY_BRACE_START_MATCH_AND_CAPTURE).filter(NonEmpty).map(function (str, i, arr) {
    let result = str;
    if (i % 2 === 0) {
      result = result.split(SEMICOLON_MATCH_AND_CAPTURE).filter(NonEmpty).map(function (semi, i_semi, arr_semi) {
        const next = arr_semi[i_semi + 1];

        if (i_semi % 2 === 0) {
          if (spacing && next && next.startsWith(';')) {
            semi = semi.replace(/:+/g, ': ');
          }
      ...
      result = result.split(CURLY_BRACE_END_MATCH_AND_CAPTURE).filter(NonEmpty).map(function (end, i_end, arr_end) {
        console.log(end, i, i_end, IL);
